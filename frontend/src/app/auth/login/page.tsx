...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try { onSuccess(await apiRequest("/auth/login", { method: "POST", body: JSON.stringify(formData) })) }
  ...
  const fbLogin = () => {
    ...
    FB.login(async (r: any) => {
      if (!r.authResponse) { setError("FB login cancelled."); return }
      setIsLoading(true); setError("")
      try { onSuccess(await apiRequest("/auth/facebook", { method: "POST", body: JSON.stringify({ access_token: r.authResponse.accessToken }) })) }
      catch (err: any) { setError(err.message || "FB login failed.") }
      finally { setIsLoading(false) }
    ...
    <div className="container flex items-center justify-center min-h-[calc(100vh-16rem)] py-12">
      <div className="w-full max-w-md space-y-8 border bg-card p-8 rounded-2xl shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button variant="outline" className="h-10" onClick={() => googleLogin()} disabled={isLoading}>
